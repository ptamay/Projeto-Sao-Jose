            ...
            <div className="modal-box" style={{ maxWidth: 420 }} onClick={e => e.stopPropagation()}>
                <div className="modal-header">
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
                        <div style={{ width: 36, height: 36, borderRadius: '50%', background: danger ? 'rgba(239,68,68,0.15)' : 'rgba(212,170,66,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={danger ? '#f87171' : 'var(--gold-400)'} strokeWidth="2">
                ...
                <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end' }}>
                    ...
                    <button className={`btn ${danger ? 'btn-danger' : 'btn-gold'}`} onClick={() => { onConfirm(); onCancel(); }}>{confirmText}</button>
                </div>
            </div>
        </div>
    );
